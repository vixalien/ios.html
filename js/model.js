...
function NElement(attributes, model) {
	...
	return new Watcher(this, permitted, model.rules.key.name);
}

function Watcher(elem, permitted = [], key) {
	this.props = [];
	this.log = [];
	this.change = {};
	NHook.call(this);
	for(prop in elem) {
		if(permitted.includes(prop)) {
			...
			throw new Error("An Unpermitted property was found: " + prop);
		}
	}
	this[key] = undefined;
	const handler = {
		...
		set: function(obj, prop, value) {
			if(prop in obj) {
				...
				obj.change[prop].push(obj[prop]);
				obj[prop] = value;
			} else {
				throw new Error("Trying to add an unpermitted property: " + prop);
			}
		}
	};
...
class NModel {
	...
	new(attributes) {
		var elem = new NElement(attributes, this);
	...
	save(key, elem) {
		...
		if (valid) {
			if (ret >= 0) {
				...
				if (typeof elem[this.rules.key.name] == "undefined") {
					if (this.elements.length > 0) {
						elem[this.rules.key.name] = this.elements[this.elements.length - 1][this.rules.key.name] + 1;
					} else {
						elem[this.rules.key.name] = 1;
					}
				}
